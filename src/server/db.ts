...
export async function syncDbWithSupabase(): Promise<void> {
  ...
  if (!isSupabaseEnabled || !supabaseClient) {
    console.log('Supabase is not enabled, using local database cache.');
    getDb();
    return;
  }
  ...
  if (error) {
    ...
    console.log('Falling back to local database cache.');
    return;
  }
  ...
  if (!hasCloudData) {
    ...
    console.log('Supabase is empty. Uploading local initial data...');
    for (const key of keys) {
      try {
        ...
        if (upsertErr) {
          ...
          console.log(`Uploaded ${key} to Supabase successfully.`);
        }
      } catch (err) {
    ...
    for (const key of keys) {
      ...
      if (row && row.data !== undefined && row.data !== null) {
        ...
        (tempDb as any)[key] = currentDb[key] !== undefined ? currentDb[key] : (key === 'cms' ? {} : []);
      }
    }
    const mergedDb = { ...currentDb, ...tempDb } as DBData;
    ensureCmsIntegrity(mergedDb);
    fs.writeFileSync(DB_FILE, JSON.stringify(mergedDb, null, 2), 'utf-8');
    inMemoryDbCache = mergedDb;
  }

  console.log('✓ Database synchronization with Supabase completed!');
}
